...
using Version = MyPlanner.Shared.Domain.ValueObjects.Version;
...
namespace MyPlanner.Plannings.Domain.PlanAggregate
{
    public class PlanProps : IProps
    {
        ...
        public Version LastVersion { get; set; } = Version.Default();
        public ReleaseDate LastReleaseDate { get; set; }
        ...
        public PlanProps(IdValueObject id, PlanCode code, IdValueObject sizeModelTypeId, Name name, Owner owner, UserId userId)
        {
            ...
            Name = name;
            LastVersion = Version.Default();
            Owner = owner;
            Items = new List<PlanItem>();
            Audit = Audit.Create(userId.GetValue());
            LastReleaseDate = ReleaseDate.Default();
    ...
    public class Plan : Entity<Plan, PlanProps>
    {
        private Plan(PlanProps props) : base(props)
        {
            if (IsNew)
            {
                var domainEvent = new PlanDraftCreatedDomainEvent(props.Id.GetValue(), props.Code.GetValue(), 
                                                             props.Name.GetValue(), Props.Owner.GetValue(), 
                                                             Props.Audit.GetValue().CreatedAt, nameof(PlanDraftCreatedDomainEvent), PlanStatus.Draft.Id);
